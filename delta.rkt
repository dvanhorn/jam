#lang racket
(require srfi/13)
;; DVH: Porting notes from Guha's code:
;; - pregexp stuff replaced with regexp
...
(define λJS-δ
  ...
  [`(str-split-strexp ,(? string? a) ,(? string? b))
   (list->jsobjhash (regexp-split (pregexp (regexp-quote b)) a))]
  [`(str-split-regexp ,(? string? a) ,(? string? b))
   (list->jsobjhash (regexp-split (pregexp b) a))]
  ...
  [`(regexp-match ,(? string? a) ,(? string? b))
   (let ([result (regexp-match (regexp b) a)])
