...
(define-metafunction JS
  ...
  [(A-norm ANS) ANS]
  [(A-norm (σ V)) (σ V)]
  [(A-norm (in-hole 𝓔 PR)) (in-hole 𝓔 PR)]
  [(A-norm (in-hole 𝓔 (clos (rec (S E) ...) ρ)))
   (A-norm (in-hole 𝓔 (rec (S (clos E ρ)) ...)))]
  [(A-norm (in-hole 𝓔 (clos (let (X E_0) E_1) ρ)))
   (A-norm (in-hole 𝓔 (let (X (clos E_0 ρ)) (clos E_1 ρ))))]
  [(A-norm (in-hole 𝓔 (clos (app E_0 E_1 ...) ρ)))
   (A-norm (in-hole 𝓔 (app (clos E_0 ρ) (clos E_1 ρ) ...)))]
  [(A-norm (in-hole 𝓔 (clos (rec-ref E_0 E_1) ρ)))
   (A-norm (in-hole 𝓔 (rec-ref (clos E_0 ρ) (clos e_1 ρ))))]
  [(A-norm (in-hole 𝓔 (clos (rec-set E_0 E_1 E_2) ρ)))
   (A-norm (in-hole 𝓔 (rec-set (clos E_0 ρ) (clos E_1 ρ) (clos E_2 ρ))))]
  [(A-norm (in-hole 𝓔 (clos (rec-del E_0 E_1) ρ)))
   (A-norm (in-hole 𝓔 (rec-del (clos E_0 ρ) (clos E_1 ρ))))]
  [(A-norm (in-hole 𝓔 (clos (set E_0 E_1) ρ)))
   (A-norm (in-hole 𝓔 (set (clos E_0 ρ) (clos E_1 ρ))))]
  [(A-norm (in-hole 𝓔 (clos (ref E) ρ)))
   (A-norm (in-hole 𝓔 (ref (clos E ρ))))]
  [(A-norm (in-hole 𝓔 (clos (deref E) ρ)))
   (A-norm (in-hole 𝓔 (deref (clos E ρ))))]
  [(A-norm (in-hole 𝓔 (clos (if E_0 E_1 E_2) ρ)))
   (A-norm (in-hole 𝓔 (if (clos E_0 ρ) (clos E_1 ρ) (clos E_2 ρ))))]
  [(A-norm (in-hole 𝓔 (clos (begin E_0 E_1) ρ)))
   (A-norm (in-hole 𝓔 (begin (clos E_0 ρ) (clos E_1 ρ))))]
  [(A-norm (in-hole 𝓔 (clos (label L E) ρ)))
   (A-norm (in-hole 𝓔 (label L (clos E ρ))))]
  [(A-norm (in-hole 𝓔 (clos (break L E) ρ)))
   (A-norm (in-hole 𝓔 (break L (clos E ρ))))]
  [(A-norm (in-hole 𝓔 (clos (try/catch E_0 X E_1) ρ)))
   (A-norm (in-hole 𝓔 (try/catch (clos E_0 ρ) X (clos E_1 ρ))))]
  [(A-norm (in-hole 𝓔 (clos (try/finally E_0 E_1) ρ)))
   (A-norm (in-hole 𝓔 (try/finally (clos E_0 ρ) (clos E_1 ρ))))]
  [(A-norm (in-hole 𝓔 (clos (throw E) ρ)))
   (A-norm (in-hole 𝓔 (throw (clos E ρ))))]
  [(A-norm (in-hole 𝓔 (clos (prim OP E ...) ρ)))
   (A-norm (in-hole 𝓔 (prim OP (clos E ρ) ...)))])
